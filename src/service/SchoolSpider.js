const citySelector = '#proline > div > div.flex-scx.mb20 > div > div:nth-child(1)';
const yearSelector = '#proline > div > div.flex-scx.mb20 > div > div:nth-child(2)';
const wenliSelector = '#proline > div > div.flex-scx.mb20 > div > div:nth-child(3)';
...
export async function getCurrentSchoolScore(currentSchoolScoreList, schoolId, schoolName, page) {
  ...
  for (const cityOption of cityOptions) {
    ...
    for (const yearOption of yearOptions) {
      ...
      for (const wenliOption of wenliOptions) {
        ...
        await new Promise(resolve => setTimeout(resolve, timeoutSelector));
        let knownSchoolScoreFields = {schoolId, schoolName, city, year, wenli};
