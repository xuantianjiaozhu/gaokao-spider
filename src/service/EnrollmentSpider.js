const citySelector = '#zs_plan > div > div.flex-scx.mb20 > div > div:nth-child(1)';
const yearSelector = '#zs_plan > div > div.flex-scx.mb20 > div > div:nth-child(2)';
const wenliSelector = '#zs_plan > div > div.flex-scx.mb20 > div > div:nth-child(3)';
const piciSelector = '#zs_plan > div > div.flex-scx.mb20 > div > div:nth-child(4)';
const enrollmentTableSelector = '#zs_plan > div > div:nth-child(2) > div > table';
const dropdownSelector = '.ant-select-dropdown:not(.ant-select-dropdown-hidden) .ant-select-dropdown-menu-item';
const pageSelector = '#zs_plan > div > div.laypage > div > ul > li.ant-pagination-next';
const timeoutSelector = 300

export async function getCurrentEnrollmentPlan(currentEnrollmentPlanList, schoolId, schoolName, page) {
    ...
    for (const yearOption of yearOptions) {
      ...
      for (const wenliOption of wenliOptions) {
        ...
        for (const piciOption of piciOptions) {
          ...
          await new Promise(resolve => setTimeout(resolve, timeoutSelector));
          let knownEnrollmentPlanFields = {schoolId, schoolName, city, year, wenli, pici};
          let currentEnrollmentPlan = await getCurrentEnrollmentPlanBySelector(page);
          currentEnrollmentPlan = currentEnrollmentPlan.map(enrollmentPlan => {
...
async function getCurrentEnrollmentPlanBySelector(page) {
  ...
  while (true) {
    const currentPageData = await page.$eval(enrollmentTableSelector, table => {
      return Array.from(table.querySelectorAll('tbody > tr')).map(row => {
        ...
        return {
          ...
          subjectRequirements: columns[4]?.textContent.trim() === '填报' ? null : columns[4]?.textContent.trim()
        };
      });
    });
